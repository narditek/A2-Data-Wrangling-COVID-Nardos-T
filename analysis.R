...
total_us_cases<-national[national$date==most_recent,3]
# Which state has had the highest number of cases?
# `state_highest_cases`
highest_cases <-max(states$cases)
states_highest_cases<-states[states$cases==highest_cases,2]
...
county_highest_cases<-counties[counties$date==(counties$date),2]
...
location_most_deaths<-coutnies[counties$deaths==max(counties$deaths),3]
...
# `date_most_cases`
dates_most_cases<-national[national$new_cases==max(national$new_cases,na.rm=TRUE),1][2]

# What was the date when the most new deaths occured?
# `date_most_deaths`
dates_most_death<-national[national$new_deaths== max(national$new_deaths,na.rm=TRUE),1][2]
...
highest_case_together<-counties%>%
  filter(date==max(date))%>%
  filter(cases == max(cases))
highest_in_each_state <- highest_case_together[3]
# What is the county with the *current* (e.g., on the most recent date)
# lowest number of deaths in each state? Your answer, stored in
# `lowest_in_each_state`, should be a *vector* of
# `location` names (the column with COUNTY, STATE).
lowest_case_together <- counties %>%
  filter(date == min(date)) %>%
  filter(cases == min(cases))
lowest_in_each_state <- lowest_case_together[3]
...
joined_states<-full_join(sum_county_to_state,states) 
...
# what is the maximum number of cases in the nation?
max_cases_states<-max(states$cases)
# what is minimum number of cases in the nation?
min_cases_states<-min(states$cases)
# what is the difference of the maximum number of cases and the minimum cases in the nation?
diff_cases_states<-(max_cases_states)-(min_cases_states)
